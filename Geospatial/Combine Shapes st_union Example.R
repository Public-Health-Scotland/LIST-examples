...
postcode_sf <- list.files("/conf/linkage/output/lookups/Unicode/Geography/Shapefiles", 
                          pattern = "PC_Cut_\\d+_\\d+",
                          full.names = TRUE) %>%
  data.frame(filename = .) %>%
  mutate(date=lapply(str_extract_all(filename, "-?\\d*\\.?\\d+"),paste,collapse="")) %>%
...
postcode_sf <- postcode_sf %>%
  ...
  mutate(true_postcode = mapply(FUN=function(a, b){gsub(a, "", b, fixed = TRUE)},sector,postcode)) %>%
  mutate(true_postcode = case_when(
...
postcode_sf <- non_split_postcodes_sf %>% 
  ...
  full_join(spd_lookup, by=c("postcode" = "pc8"),relationship="one-to-one") %>%
  
  dplyr::select(hb2019 = hb2019, 
