...
get_travel_areas <- function(
  ...
  if (length(latitude) > 10) {
    cli::cli_inform(
      "Calculating travel areas for {length(latitude)} areas may take a while, fewer areas will be quicker."
    )
  }
  # Get travel areas
  iso <- purrr::map2(
    longitude,
    latitude, # Iterate over each point (slow)
  ...
  if (inherits(iso$geometry, "sfc_MULTIPOLYGON")) {
    ...
    st_geometry(iso) <- st_sfc(
      ...
      crs = 4326
    ) # Add the polygons together to make mapping simpler
  }
...
get_locations_within <- function(locations, iso) {
  ...
  long_col <- stringr::str_subset(
    names(locations),
    stringr::regex("^long?(:?itude)?$", ignore_case = TRUE)
  )
  lat_col <- stringr::str_subset(
    names(locations),
    stringr::regex("^lat(:?itude)?$", ignore_case = TRUE)
  )

  # Notify user which columns are being taken as long and lat
  cli::cli_inform(
    "Assuming {.val {long_col}} and {.val {lat_col}} are longitude and latitude respectively."
  )
